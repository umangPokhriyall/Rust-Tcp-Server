...
model, taken from committed CSVs and histogram dumps under `bench/results/`.
...
**Host** (recorded in `bench/results/profiles/README.md`):

| Property | Value |
|---|---|
| CPU | 11th Gen Intel Core i5-1135G7 @ 2.40 GHz |
| Cores / threads | 4 physical / 8 logical |
| RAM | 8 GiB (`MemTotal: 7,322,952 kB`, `bench/results/c10k_README.md`) |
...
inverse_1_minus_p` — e.g. `bench/results/epoll-et_r40000_c1000.hgrm`) and
...
50000` rps, 10 s per point (`bench/results/sweep.log`), appending rows to
`bench/results/<model>.csv`. Because the load is rate-capped, a model that
...
fd count, writing the resource curve to `bench/results/c10k_<model>.log` and a
verdict row to `bench/results/c10k_summary.csv`.
...
models, into `bench/results/profiles/`.
...
- **Single-host loadgen contention.** The load generator runs on the same 8
  ...
  threads on 8 cores — `bench/results/profiles/README.md`) and is the honest
...
- **C10K capped at 8000 connections.** The loadgen cannot `pthread_create`
  ...
  reservations fail with `EAGAIN` (`bench/results/c10k_README.md`). 8000 is the
  ...
  the server was tested. Both are explained in `bench/results/c10k_README.md`.
- **Top-down microarchitecture unavailable.** `perf_event_paranoid = 4` on this
  ...
  available, so `perf stat` opens no events (`bench/results/profiles/
...
Throughput and latency at C=100 are from the sweep CSVs (`bench/results/
<model>.csv`, offered rate 20000 rps). The C10K rung is from the C10K capture
at 8000 connections / 50000 rps offered (`bench/results/c10k_<model>.csv` and
`bench/results/c10k_summary.csv`). Syscalls/req and ctx-switches/req are from
`bench/results/profiles/summary.csv` and were measured only for the three
...
Sources: C=100 rows from each `bench/results/<model>.csv` (row `rate=20000,
connections=100`); C=8000 rows from `bench/results/c10k_<model>.csv` and
`bench/results/c10k_summary.csv`; syscalls/req and ctx-switches/req from
...
**half** the syscalls/req of `epoll-et` (2.015 vs 4.024, `bench/results/
...
![Latency distribution at C=1000](../bench/results/distribution_c1000.png)

*Source: the `*_r40000_c1000.hgrm` dumps in `bench/results/`.*
...
*Source: `bench/results/multireactor_scaling.csv`. See §6 for why the
...
(`bench/results/scaling.log`, `bench/results/multireactor_scaling.csv`):
...
ctx-switches/req profile (`bench/results/profiles/summary.csv`); the other
...
> [`bench/results/iterative.csv`]; at C=8000 it does not complete — the single
> serving thread cannot drain 8000 connections inside the budget and the point
> is recorded `saturated` [`bench/results/c10k_summary.csv`]. Not profiled at
...
> C=100 it serves 20000 req/s (p99 = 15055 µs) [`bench/results/forking.csv`];
> at C=8000 the server process panics spawning the child for connection N with
> `Os { code: 11, kind: WouldBlock, message: "Resource temporarily unavailable" }`
> [`bench/results/c10k_server_forking.log`], the `EAGAIN`-on-clone wall.
...
> µs) [`bench/results/preforked.csv`]; at C=8000 the 8 workers cannot drain the
> connections and the point is `saturated` [`bench/results/c10k_summary.csv`].
...
> (p99 = 5695 µs) [`bench/results/thread-per-conn.csv`]; at C=8000 it panics
...
> [`bench/results/c10k_server_thread-per-conn.log`, `bench/results/
> c10k_thread-per-conn.log`, `bench/results/c10k_summary.csv`]. Not profiled;
...
> [`bench/results/thread-pool.csv`]; at C=8000 it stays up but sheds heavily —
...
> `bench/results/c10k_thread-pool.log`) because the pool is bounded — that is
...
> (p99 = 11495 µs) [`bench/results/poll.csv`]; at C=8000 it carries all 8000
> connections at the full 50000 rps with 0 errors, but at p50 = 9975 µs and
> p99 = 293631 µs [`bench/results/c10k_poll.csv`], RSS rising to 10176 KiB and
> fds to 8004 [`bench/results/c10k_poll.log`]. Not profiled; top-down
...
> (p99 = 22639 µs) [`bench/results/epoll-lt.csv`]; at C=8000 it carries all 8000
> at 50000 rps with 0 errors but the worst median of any surviving model —
> p50 = 457215 µs (457 ms), p99 = 1514495 µs (1.5 s)
> [`bench/results/c10k_epoll-lt.csv`], RSS flat at ≈ 10 MiB
> [`bench/results/c10k_epoll-lt.log`]. Not profiled at syscall level; top-down
...
> At C=100 it serves 20000 req/s (p99 = 23055 µs) [`bench/results/
...
> rung — and p99 = 471551 µs [`bench/results/c10k_epoll-et.csv`], RSS flat at
> 10056 KiB, fds returning to 5 after drain [`bench/results/c10k_epoll-et.log`].
> The profile is **4.024 syscalls/req** (1×`epoll_wait`, 1×`sendto`,
> 2×`recvfrom`) and **1.008 ctx-switches/req** [`bench/results/profiles/
> summary.csv`, `bench/results/profiles/strace_epoll-et.txt`]. The second
...
> [`bench/results/event-loop.csv`]; at C=1000 that inverts — p99 = 28175 µs vs
> epoll-et's 214783 µs [`bench/results/event-loop.csv`,
> `bench/results/epoll-et.csv`]; at C=8000 it carries all 8000 at 50000 rps
> with 0 errors, p50 = 63519 µs, p99 = 375551 µs
> [`bench/results/c10k_event-loop.csv`], RSS flat at ≈ 10 MiB
> [`bench/results/c10k_event-loop.log`]. Not profiled at syscall level; top-down
...
> [`bench/results/multireactor.csv`]; at C=8000 it carries all 8000 at 50000 rps
> with 0 errors at the best median of any surviving model, p50 = 86 µs (p99 =
> 366335 µs) [`bench/results/c10k_multireactor.csv`], RSS flat at ≈ 10 MiB, fds
> returning to 19 after drain [`bench/results/c10k_multireactor.log`]. The
...
> [`bench/results/profiles/summary.csv`, `bench/results/profiles/
...
> [`bench/results/io-uring.csv`]; at C=1000 it sheds load — 11270.3 req/s with
> 287297 errors, though at low served-latency p99 = 72127 µs
> [`bench/results/io-uring.csv`]; at C=8000 it serves 18402.4 req/s with 947927
> errors, p99 = 16463 µs on what it serves [`bench/results/c10k_io-uring.csv`].
...
> ctx-switches/req** [`bench/results/profiles/summary.csv`,
> `bench/results/profiles/strace_io-uring.txt`] — half the syscalls/req of
...
- **Syscalls/req: 2.015 (io-uring) vs 4.024 (epoll-et)** — a 2.00× reduction
  ...
  [`bench/results/profiles/strace_epoll-et.txt`,
...
- **Ctx-switches/req: 1.006 (io-uring) vs 1.008 (epoll-et)** — effectively
  identical [`bench/results/profiles/summary.csv`]. Both park once per request
...
- **Throughput at scale: epoll-et wins; io_uring sheds load.** At C=1000,
  ...
  (`bench/results/epoll-et.csv`, `bench/results/io-uring.csv`). The two adopt
...
`bench/results/c10k_summary.csv`), the models split cleanly into those that
...
`bench/results/c10k_<model>.log` curves. RSS sits at ≈ 10 MiB to carry 8000
...
30 s [`bench/results/c10k_summary.csv`] — its single ring parking on
...
- **forking** and **thread-per-conn**: the server process **panics** allocating
  ...
  [`bench/results/c10k_server_forking.log`,
  `bench/results/c10k_server_thread-per-conn.log`] — `EAGAIN` on clone once
  thread-stack commits exceed the host limit. `thread-per-conn` reaches RSS =
  121104 KiB (118 MiB) and 4608 fds before it dies
  [`bench/results/c10k_thread-per-conn.log`], the per-thread stack cost made
  visible.
- **iterative** and **preforked**: recorded `saturated` — the 1 (resp. 8)
  ...
  the budget, so the loadgen never completes [`bench/results/c10k_summary.csv`,
  empty `bench/results/c10k_iterative.csv` / `c10k_preforked.csv`].
- **thread-pool**: survives without dying (bounded RSS ≈ 3 MiB) but sheds 67 %
  ...
  [`bench/results/c10k_thread-pool.csv`] — a bounded blocking pool can serve
...
- **The reactor `event-loop` is not a clean zero-cost abstraction over bare
  ...
  p50 = 63519 µs vs epoll-et 157 µs) [`bench/results/event-loop.csv`,
  `bench/results/epoll-et.csv`, `bench/results/c10k_event-loop.csv`,
  `bench/results/c10k_epoll-et.csv`]. The abstraction changes the latency
...
- **Purpose-built `io_uring` did not win the workload on throughput.** The
  syscall hypothesis held exactly (2.015 vs 4.024/req, `bench/results/profiles/
  ...
  C=8000 [`bench/results/io-uring.csv`, `bench/results/c10k_io-uring.csv`] —
...
- **`epoll-lt` collapses by latency at C10K while reporting zero errors.** It
  ...
  [`bench/results/c10k_epoll-lt.csv`] — a half-second median — against
  edge-triggered epoll's 157 µs at the identical rung
  [`bench/results/c10k_epoll-et.csv`]. The level-triggered re-notification cost
...
- **`poll` survived C10K.** The O(n)-scan baseline carried 8000 connections at
  50000 rps with 0 errors [`bench/results/c10k_poll.csv`] — its weakness is the
...
- **The multireactor throughput scaling factor came out flat at 1.0×.** This is
  ...
  two), not in throughput (§6, `bench/results/multireactor_scaling.csv`).
- **multireactor carries the highest ctx-switches/req of the three signal
  models** (1.153 vs ≈ 1.006–1.008) [`bench/results/profiles/summary.csv`], from
...
| C=8000 (C10K rung) latency/throughput/errors | `bench/results/c10k_<model>.csv` |
...
| forking / thread-per-conn failure panics | `bench/results/c10k_server_forking.log`, `bench/results/c10k_server_thread-per-conn.log` |
| C10K = 8000 (not 10000) justification | `bench/results/c10k_README.md` |
| syscalls/req (4.024 / 4.059 / 2.015) | `bench/results/profiles/summary.csv`, `bench/results/profiles/strace_<model>.txt` |
| ctx-switches/req (1.008 / 1.153 / 1.006) | `bench/results/profiles/summary.csv`, `bench/results/profiles/ctx_<model>.txt` |
| syscall breakdown (epoll_wait/recvfrom/sendto; io_uring_enter) | `bench/results/profiles/strace_epoll-et.txt`, `bench/results/profiles/strace_io-uring.txt`, `bench/results/profiles/README.md` |
| perf top-down omission (paranoid=4) | `bench/results/profiles/README.md` |
| multireactor scaling table | `bench/results/multireactor_scaling.csv`, `bench/results/scaling.log` |
| latency distribution plots (C=100, C=1000) | `bench/results/distribution_c100.png`, `bench/results/distribution_c1000.png` (from the `*_r20000_c100.hgrm` / `*_r40000_c1000.hgrm` dumps) |
...
| host CPU / RAM / kernel | `bench/results/profiles/README.md`, `bench/results/c10k_README.md` |
| sweep concurrency/rate/duration grid | `bench/results/sweep.log` |
